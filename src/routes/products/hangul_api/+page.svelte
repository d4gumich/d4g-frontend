<script>
  ...
  function resetCheckboxes() {
    checkboxes.update(items =>
      items.map(checkbox => ({ ...checkbox, checked: false }))
    );
    num_keywords.set(0);
  }
...
let allSelected = $state(false); // Tracks the toggle state

function toggleSelectAll() {
  allSelected = !allSelected; // Toggle the state

  checkboxes.update(items => 
    items.map(checkbox => ({ ...checkbox, checked: allSelected }))
  );
}
function updateCheckbox(id, newValue) {
  // Use the store's update method to return a new array
  checkboxes.update(items =>
    items.map(item =>
  ...
  <div class="container">
    {#if !showResults}
      ...
      <div class="param-container">
        <h1 class="text-container-heading">
            Select parameters to specify if certain document attributes should be extracted:
        </h1>
        <div class="num-keywords">
          <p class="param-text">Number of keywords to extract: </p>
          <label>
            <input
              type="number"
              value={$num_keywords}
              ...
              oninput={e => {
                ...
                num_keywords.set(inputVal === "" ? 0 : parseInt(inputVal, 10));
              }}
            />
          </label>
        </div>
        ...
        <div class="btn-group">
            {#each $checkboxes.slice(0,10) as checkbox (checkbox.id)}
            <label class="param-text">
              <input
                type="checkbox"
                bind:checked={checkbox.checked}
                onchange="{e => updateCheckbox(checkbox.id, e.target.checked)}"
              />
              {checkbox.label}
            </label>
          {/each}
        ...
        <div class="btn-group">
          {#each $checkboxes.slice(10,15) as checkbox (checkbox.id)}
          <label class="param-text">
            <input
              type="checkbox"
              bind:checked={checkbox.checked}
